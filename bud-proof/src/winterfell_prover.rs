use winter_math::{fields::f128::BaseElement, ExtensionOf, FieldElement, ToElements};
use winterfell::{
    matrix::ColMatrix, AirContext, Assertion, AuxTraceRandElements,
    ConstraintCompositionCoefficients, DefaultConstraintEvaluator, DefaultTraceLde,
    EvaluationFrame, ProofOptions, Prover as WinterProver, StarkDomain, Trace, TraceInfo,
    TraceLayout, TracePolyTable, TransitionConstraintDegree,
};
...
impl winterfell::Air for BudAir {
    ...
    fn new(trace_info: TraceInfo, _pub_inputs: PubInputs, options: ProofOptions) -> Self {
        ...
        let context =
            AirContext::new_multi_segment(trace_info, degrees, aux_degrees, 2, 2, options);
        Self { context }
    }
    ...
    fn evaluate_transition<E: FieldElement<BaseField = Self::BaseField>>(
        ...
        result[7] =
            cur[COL_IS_JNZ] * (next_pc - (jnz_cond * (pc + imm) + (one - jnz_cond) * (pc + one)));
        ...
        E: FieldElement<BaseField = Self::BaseField> + ExtensionOf<F>,
    {
        ...
        result[0] =
            anxt[0] - acur[0] * (E::from(is_cpu_acc) * cpu_packet + (E::ONE - E::from(is_cpu_acc)));

        let is_reg = cur[COL_REG_ACTIVE];
        let reg_packet = get_term(
            cur[COL_REG_CLK],
            cur[COL_REG_IDX],
            cur[COL_REG_VAL],
            cur[COL_REG_IS_WRITE],
        );
...
impl Trace for BudTrace {
    ...
    fn build_aux_segment<
        E: FieldElement<BaseField = Self::BaseField> + ExtensionOf<Self::BaseField>,
    >(
        &mut self,
        ...
        let get_term =
            |clk: BaseElement, idx: BaseElement, val: BaseElement, write: BaseElement| -> E {
                alpha
                    + E::from(clk) * beta
                    + E::from(idx) * beta2
                    + E::from(val) * beta3
        ...
        for i in 0..n - 1 {
            let is_cpu = self.main_trace.get_column(COL_IS_ADD)[i]
                + self.main_trace.get_column(COL_IS_SUB)[i]
                + self.main_trace.get_column(COL_IS_MUL)[i]
                + self.main_trace.get_column(COL_IS_EQ)[i]
                + self.main_trace.get_column(COL_IS_LT)[i]
                + self.main_trace.get_column(COL_IS_JMP)[i]
                + self.main_trace.get_column(COL_IS_JNZ)[i]
                + self.main_trace.get_column(COL_IS_LOAD)[i]
                + self.main_trace.get_column(COL_IS_ASSERT)[i]
                + self.main_trace.get_column(COL_IS_LOG)[i];

            if is_cpu != BaseElement::ZERO {
                let t1 = get_term(
                    self.main_trace.get_column(COL_CLK)[i],
                    self.main_trace.get_column(COL_RS1_IDX)[i],
                    self.main_trace.get_column(COL_RS1_VAL)[i],
                    BaseElement::ZERO,
                );
                let t2 = get_term(
                    self.main_trace.get_column(COL_CLK)[i],
                    self.main_trace.get_column(COL_RS2_IDX)[i],
                    self.main_trace.get_column(COL_RS2_VAL)[i],
                    BaseElement::ZERO,
                );
                let t3 = get_term(
                    self.main_trace.get_column(COL_CLK)[i],
                    self.main_trace.get_column(COL_RD_IDX)[i],
                    self.main_trace.get_column(COL_RD_VAL_NEW)[i],
                    BaseElement::ONE,
                );
            ...
            if is_reg != BaseElement::ZERO {
                let t = get_term(
                    self.main_trace.get_column(COL_REG_CLK)[i],
                    self.main_trace.get_column(COL_REG_IDX)[i],
                    self.main_trace.get_column(COL_REG_VAL)[i],
                    self.main_trace.get_column(COL_REG_IS_WRITE)[i],
                );
...
impl Prover {
    pub fn generate_trace(trace: &[bud_vm::Step]) -> BudTrace {
        ...
        for (i, step) in trace.iter().enumerate() {
            ...
            match op {
                ...
                0x11 => {
                    ...
                    cols[COL_JNZ_COND][i] = if step.src1_val != 0 {
                        BaseElement::ONE
                    } else {
                        BaseElement::ZERO
                    };
        ...
        for (i, e) in events.iter().enumerate() {
            ...
            cols[COL_REG_IS_WRITE][i] = if e.is_write {
                BaseElement::ONE
            } else {
                BaseElement::ZERO
            };
            ...
            Ok(stark_proof) => {
                ...
                winterfell::verify::<
                    ...
                    winterfell::crypto::DefaultRandomCoin<
                        winterfell::crypto::hashers::Blake3_256<BaseElement>,
                    >,
                >(stark_proof, PubInputs { num_steps }, &min_opts)
                .is_ok()
            }
            Err(_) => false,
        }
